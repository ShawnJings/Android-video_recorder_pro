...
void RecordingPublisher::registerAdaptiveBitrateCallback(
        int (*on_adaptive_bitrate_callback)(PushVideoQuality videoQuality, void *context),
        void *context) {
    if (NULL != rateFeedback) {
        rateFeedback->registerAdaptiveBitrateCallback(on_adaptive_bitrate_callback, context);
    }
}
...
void RecordingPublisher::registerHotAdaptiveBitrateCallback(
        ...
        void *context) {
    if (NULL != rateFeedback) {
        rateFeedback->registerHotAdaptiveBitrateCallback(hot_adaptive_bitrate_callback, context);
    }
}
...
void RecordingPublisher::registerStatisticsBitrateCallback(
        ...
        void *context) {
    if (NULL != rateFeedback) {
        rateFeedback->registerStatisticsBitrateCallback(statistics_bitrate_callback, context);
    }
}
...
int RecordingPublisher::detectTimeout() {
    if (platform_4_live::getCurrentTimeMills() - sendLatestFrameTimemills > publishTimeout) {
        ...
        LOGI("RecordingPublisher::interrupt_cb callback time out ... queue size:%d", queueSize);
...
int RecordingPublisher::init(char *videoOutputURI,
    ...
    rateFeedback = PublisherRateFeedback::GetInstance();
    LOGI("PublisherRateFeedback::GetInstance()");
    rateFeedback->init(audioBitRate, qualityStrategy, configMap);
    LOGI("rateFeedback->init");
    return 1;
}
...
int RecordingPublisher::encode() {
    ...
    if (ret < 0 && VIDEO_QUEUE_ABORT_ERR_CODE != ret && AUDIO_QUEUE_ABORT_ERR_CODE != ret &&
        !isInterrupted()) {
        /**
         * May Be :
         *  Send Data Time Out
         *  Net Work DisConnect
         *  Broken Pipe( Server Disconnect Our Pipe)
         */
...
int RecordingPublisher::interleavedWriteFrame(AVFormatContext *s, AVPacket *pkt) {
    ...
    if (ret == 0) {
        if (NULL != rateFeedback) {
            rateFeedback->publishPktSuccess(publishSize);
        }
    }
    return ret;
}
