...
void *HWEncoderAdapter::encoderThreadCallback(void *myself) {
    ...
    LOGE("encoderThreadCallback begin");
    encoder->encodeLoop();
    pthread_exit(0);
    LOGE("encoderThreadCallback end");
...
//	env->CallVoidMethod(obj, reConfigureFunc, videoBitRate);
...
//	LOGI("size is %d", size);
    ...
    //统计发送和编码率等数据，码率自适应
    PublisherRateFeedback::GetInstance()->statisticCompressData(timeMills, size,
                                                                (1.0)*packetPool->getRecordingVideoPacketQueueSize()/60);
