...
void RecordProcessor::setAudioEffect(AudioEffect* audioEffect) {
	if(NULL != audioEncoder){
		audioEncoder->setAudioEffect(audioEffect);
	} else{
		this->audioEffect = audioEffect;
	}
}

void RecordProcessor::initAudioBufferSize(int sampleRate, int audioBufferSizeParam, AudioEffect* audioEffect) {
...
void RecordProcessor::flushAudioBufferToQueue() {
	if (audioSamplesCursor > 0) {
		if(NULL == audioEncoder){
			...
			audioEncoder->init(packetPool, audioSampleRate, audioChannels, audioBitRate, audioCodecName, audioEffect);
		}
